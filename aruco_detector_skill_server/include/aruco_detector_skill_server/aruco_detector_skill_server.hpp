...
using ArucoDetectorSkill = aruco_detector_skill_msgs::action::ArucoDetectorSkill;
using GoalHandleArucoDetectorSkill = rclcpp_action::ServerGoalHandle<ArucoDetectorSkill>;

/**
 * @brief ArucoDetectorSkillServer class that provides a server for handling diet estimation skills.
 *
 * This class defines a ROS 2 action server that listens for diet estimation skill requests, executes
 * the skill, and handles goal acceptance, cancellation, and feedback.
 */
...
private:
  ...
  /**
    ...
    *
    * @throws std::runtime_error If directory creation fails due to filesystem errors
   ...
   */
  void handle_accepted(const std::shared_ptr<GoalHandleArucoDetectorSkill> _goal_handle);
   ...
   */
  void execute(const std::shared_ptr<GoalHandleArucoDetectorSkill> _goal_handle);

  /**
   * @brief Publishes feedback for an ongoing goal
   *
   * @param _goal_handle The goal handle of the goal for which feedback is published
   ...
   */
  bool check_preemption(const std::shared_ptr<GoalHandleArucoDetectorSkill> _goal_handle);
   ...
   * @param _goal_handle The goal handle to check for cancellation
   */
  void is_cancelled(const std::shared_ptr<GoalHandleArucoDetectorSkill> _goal_handle);
};
